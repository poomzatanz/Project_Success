...
<div id="wrapper">
    ...
    <?php
	 $conn = new mysqli("db4free.net","poomzaadekd2023","poomzaadekd2023","dekdanalatic2023");
      ...
      $totol3 = array();
      $totols = array();
	$sql = "SELECT count(*) as num ,SUBSTRING(time, 4, 2) as month FROM pre_web WHERE topic_acc = 'positive' GROUP BY SUBSTRING(time, 4, 2) ORDER by SUBSTRING(time, 4, 2);";
    ...
    $sql = "SELECT count(*) as num ,SUBSTRING(time, 4, 2) as month FROM pre_web WHERE topic_acc = 'negative' GROUP BY SUBSTRING(time, 4, 2) ORDER by SUBSTRING(time, 4, 2);";
    $result = $conn->query($sql);
    /* If there are results from database push to result array */
    ...
    $sql = "SELECT count(*) as num ,SUBSTRING(time, 4, 2) as month FROM pre_web WHERE topic_acc = 'normal' GROUP BY SUBSTRING(time, 4, 2) ORDER by SUBSTRING(time, 4, 2);";
    $result = $conn->query($sql);
    /* If there are results from database push to result array */
    ...
    $sql = "SELECT count(*) as num ,topic_acc as month FROM pre_web GROUP BY topic_acc;";
    $result = $conn->query($sql);
    /* If there are results from database push to result array */
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="d-flex no-block">
                            <div>
                                <h2></h2>
                                <h1 data-shadow='dang!'>วิเคราะห์หัวข้อกระทู้</h1>
                            </div>
                            <div class="ml-auto">
            ...
            <div class="col-lg-4">
                <div class="card">
                    <div class="card-body">
                        ...
                        <ul class="nav navbar-nav">
                        <br><br>
                        <li class="text-right" style="color:#fff ;font-size:25px;"><a href="acc_topic.php">ดูประสิทธิภาพ</a></li>
                            <div class="ml-auto">
...
options: {
 ...
 onClick:function(e){
	...
	window.location="table_pos.php?id="+this.data.labels[selectedIndex];
