...
<div id="wrapper">
 ...
 $conn = new mysqli("db4free.net","poomzaadekd2023","poomzaadekd2023","dekdanalatic2023");
	...
	$sql = "SELECT count(*) as num ,web.group as month FROM web WHERE topic_acc = 'positive' GROUP BY web.group ORDER by web.group";
    ...
    $sql = "SELECT count(*) as num ,web.group as month FROM web WHERE topic_acc = 'negative' GROUP BY web.group ORDER by web.group";
    $result = $conn->query($sql);
    /* If there are results from database push to result array */
    ...
    $sql = "SELECT count(*) as num ,web.group as month FROM web WHERE topic_acc = 'normal' GROUP BY web.group ORDER by web.group";
    $result = $conn->query($sql);
    /* If there are results from database push to result array */
    ...
    $sql = "SELECT count(*) as num ,web.group as month FROM web GROUP BY web.group;";
    $result = $conn->query($sql);
    /* If there are results from database push to result array */
    ...
    <div class="row" >
        <div class="c">
            ...
            <form class="form-inline" action="group_s.php" method="get">
                            ...
                            <select name="name">
                                <option value="" selected="selected" disabled="disabled">กรุณาเลือกกลุ่ม</option>
                                <?php
                                    $strSQL = "SELECT count(*) as num ,web.group as month FROM web  GROUP BY web.group ORDER by web.group;";
                                    ...
                                    while($objResuut = $objQuery->fetch_assoc())
		                        	    {
			                        ?>
			                            <option value="<?php echo $objResuut["month"];?>"><?php echo "กลุ่ม ".$objResuut["month"];?></option>
                                        <?php
                                        }
                                        ?>
                            </select>
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="d-flex no-block">
                            <div>
                                <h2></h2>
                                <h1 data-shadow='dang!'>การจัดกลุ่ม</h1>
                                <?php //echo $lable1; ?>
                            </div>
                            <div class="ml-auto">
 ...
 datasets: [ 
		 {
             ...
             backgroundColor: ["#FF33FF","#00FFFF","#B8860B","#e8c3b9", "#8e5ea2","#EC4653"]
...
options: {
 ...
 onClick:function(e){
	...
	window.location="group_s.php?name="+this.data.labels[selectedIndex];
